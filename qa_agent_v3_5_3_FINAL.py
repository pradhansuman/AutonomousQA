...
REPORT_DIR = Path("qa_v3_5_3_report")
...
class QAAgent:
    ...
    def fingerprint(self, e):
        ...
        return {
            ...
            "aria_label":
                clean(e.get("aria_label")),

            "placeholder":
                clean(e.get("placeholder")),
            ...
            "input_type":
                clean(e.get("input_type")),

            "title":
                clean(e.get("title")),

            "aria_valuenow":
                clean(e.get("aria_valuenow")),
        }
    ...
    async def resolve(self, page, fp):
        """
        Strict semantic resolver.

        IMPORTANT:
        Never fall back to an arbitrary element of the same HTML tag.
        A failed semantic match is a resolution failure, not permission
        ...
        input_type = clean(fp.get("input_type")).lower()

        candidates = []

        # ----------------------------------------------------
        # Build only semantically meaningful candidates.
        # ----------------------------------------------------
        ...
        # Remove duplicates by DOM identity where possible.
        unique = []
        ...
        for item in matches:
            ...
            for existing in unique:
                try:
                    same = await item.evaluate(
                        """(a, b) => a === b""",
                        await existing.evaluate_handle(
                            "(e) => e"
                        )
                    )
                    if same:
        ...
        for item in unique:
            try:
                tag = (
                    await item.get_attribute("tagName")
                )
            except Exception:
                tag = None

            try:
                actual_tag = await item.evaluate(
            ...
            # Text input must actually be editable text-like input.
            if semantic == "text_input":
                allowed = {
                    "",
                    "text",
            ...
            # Slider must actually be range or ARIA slider.
        ...
        # ----------------------------------------------------
        # CRITICAL:
        # Ambiguous semantic matches are rejected.
        # We do NOT pick the first arbitrary element.
        # ----------------------------------------------------
...
async def main():
    print("\n*** RUNNING qa_agent_v3_5_1_FINAL.py — VERSION 3.5.3 FINAL ***\n")
    print("\n*** RUNNING qa_agent_v3_5_1_FINAL.py — VERSION 3.5.3 FINAL ***\n")
