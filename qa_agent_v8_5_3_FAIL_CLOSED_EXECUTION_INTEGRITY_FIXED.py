...
# ============================================================================
# V8.5.3 — TRUE OBSERVABILITY / EVIDENCE CORRELATION
# ============================================================================
V852_VERSION = "8.5.2"
...
def _v85_write_truth(model, truth):
    d=Path("qa_v8_5_3_report"); d.mkdir(parents=True,exist_ok=True)
    mp=d/"qa_business_outcome_verification_v8_5_3.json"; tp=d/"qa_v8_5_3_business_outcome_truth.json"
    ...
    print("\n"+"="*70); print("🧭 V8.5.3 TRUE OBSERVABILITY / BUSINESS-OUTCOME TRUTH"); print("="*70)
    ...
    print(f"\n   {'🟢' if truth['pass'] else '🔴'} V8.5.3 EVIDENCE TRUTH GATE: {'PASS' if truth['pass'] else 'FAIL'}"); print("="*70); print(f"📄 Outcome verification: {mp}"); print(f"📄 Outcome truth: {tp}")


# ============================================================================
# V8.5.3 — FAIL-CLOSED AUTONOMOUS EXECUTION INTEGRITY
# ============================================================================
#
...
def _v853_write_integrity_artifact(payload):
    report_dir = Path("qa_v8_5_3_report")
    report_dir.mkdir(parents=True, exist_ok=True)
    path = report_dir / "qa_execution_integrity_v8_5_3.json"
    path.write_text(
...
def _v853_print_blocked_truth(payload, artifact):
    print("\n" + "=" * 70)
    print("🛡️ V8.5.3 FAIL-CLOSED EXECUTION INTEGRITY")
    print("=" * 70)
...
def _v853_fail_closed_if_discovery_invalid(agent, target):
    if not _v853_target_is_nonempty(target):
        return False

    if not _v853_discovery_failed(agent):
        return False

    # Clear anything that may have been populated from memory/baseline.
    _v853_clear_current_run_state(agent)

    payload = _v853_integrity_artifact(
        target,
        discovery_ok=False,
        blocked_reason="TARGET_DISCOVERY_FAILED_OR_RETURNED_ZERO_PAGES",
    )
...
async def _v853_preflight(url, headless):
    """Establish current-run reachability before the legacy agent can emit claims."""
    try:
        async with async_playwright() as pw:
            ...
            has_dom = bool(body_text.strip()) or bool(await page.locator("body").count())
            await browser.close()
            return {
                "ok": True,
                "status": status,
...
async def main():
    ...
    print("\n*** RUNNING qa_agent_v8_5_3_FAIL_CLOSED_EXECUTION_INTEGRITY.py — VERSION 8.5.3 ***\n")

    if not url.startswith(("http://", "https://")):
        print("❌ Invalid URL")
        sys.exit(1)

    # CRITICAL V8.5.3 RULE:
    ...
    print("🛡️ V8.5.3 CURRENT-RUN DISCOVERY PREFLIGHT")
    preflight = await _v853_preflight(url, args.headless)

    if not preflight.get("ok"):
        payload = _v853_integrity_artifact(
            target,
            discovery_ok=False,
            blocked_reason="TARGET_DISCOVERY_PREFLIGHT_FAILED",
        )
        payload["preflight"] = preflight
        artifact = _v853_write_integrity_artifact(payload)
        _v853_print_blocked_truth(payload, artifact)
        print("🔴 V8.5.3 STOP: agent execution was NOT started.")
        raise SystemExit(3)
    ...
    if _v853_fail_closed_if_discovery_invalid(agent, target):
        raise SystemExit(3)
