#!/usr/bin/env python3
"""
V8.7.13 - DISCOVERY ROBUSTNESS + SEMANTIC MODEL FIX
...
class Agent:
    ...
    async def navigate(self,page,url):
        ...
        navigation remains pending. V8.7.13 therefore separates:
          1. document commit,
          2. DOM attachment,
    ...
    async def discover(self,browser):
        self.log("="*70)
        self.log("🗺️ V8.7.13 CURRENT-RUN DISCOVERY")
    ...
    def build_behaviors(self):
        ...
        """
        self.log("="*70)
        self.log("🧠 V8.7.13 CANONICAL SEMANTIC BEHAVIOR MODEL")
    ...
    async def execute_safe_navigation(self,browser):
        ...
        V8.7.13 uses the Element attributes directly and deduplicates by
        canonical destination surface.
        """
        self.log("="*70)
        self.log("🧪 V8.7.13 SAFE NAVIGATION EXECUTION")
    ...
    async def _execute_one_semantic(self, page, loc, semantic, label):
        ...
        if s in ("TEXT_INPUT", "TEXTAREA"):
            value = "QA_V8_7_13_PROBE"
            await loc.fill(value)
            observed = await loc.input_value()
            if observed != value:
                raise RuntimeError(f"input observation mismatch: {observed!r}")
    ...
    async def execute_semantic_behaviors(self, browser):
        """V8.7.13 targeted semantic execution with bounded recovery."""
        self.log("="*70)
        self.log("🧪 V8.7.13 TARGETED SEMANTIC EXECUTION")
        ...
        self.log(
            f"🧪 V8.7.13 SEMANTIC EXECUTION COMPLETE | "
    ...
    def truth(self,discovery_ok):
        ...
        self.log("="*70)
        self.log("🧭 V8.7.13 SINGLE CANONICAL TRUTH")
        self.log("="*70)
        for k,v in truth.items():
            if k=="gates": continue
            self.log(f"{k.replace('_',' ').title():30}: {v}")
        self.log("-"*70)
        self.log("🛡️ V8.7.13 TRUTH GATES")
        self.log("-"*70)
        for k,v in gates.items():
            self.log(f"{k:30} : {'PASS' if v else 'FAIL'}")
        self.log("-"*70)
        self.log(
            "🟢 V8.7.13 RELEASE TRUTH GATE: PASS"
            if release else
            "🔴 V8.7.13 RELEASE TRUTH GATE: FAIL\n"
    ...
    async def run(self):
        self.log("="*70)
        self.log("🚀 V8.7.13 DISCOVERY ROBUSTNESS + SEMANTIC QA")
        self.log("="*70)
