...
""".format(
    ...
    def _v70_print_mission(self):
        ...
        print("\n")
        print("=" * 70)
        print("🎯 V8.2.5 AUTONOMOUS QA MISSION")
        print("=" * 70)
    ...
    def _v824_mission_truth(self):
        ...
        V8.2.5 could print 100% while reporting zero behaviors. V8.2.5
    ...
    def _v76_print(self):
        ...
        print("\n" + "=" * 70)
        print("🔄 V8.2.5 EXECUTABLE TEST-INTENT INTELLIGENCE")
        print("=" * 70)
    ...
    async def run(self):
        ...
        if mission_truth["status"] == "NOT_MEASURABLE":
            print("\n🛡️ V8.2.5 MISSION TRUTH GATE: PASS")
    ...
    #
    # V8.2.5 produces an auditable quality decision without
    # storing credentials, tokens, cookies, passwords, or secrets.
    # ========================================================

    def _v78_execution_truth(self):
        # V8.2.5: quality truth is derived from the same classified
    ...
    def _v78_write_quality_truth(self):
        ...
        try:
            ...
            print(
                f"\n⚠️ V8.2.5 quality-truth artifact "
                f"write skipped: {exc}"
            )
    ...
    def _v81_print_intent_engine(self):
        ...
        print("\n")
        print("=" * 70)
        print("🧠 V8.2.5 AUTONOMOUS TEST-INTENT ENGINE")
        print("=" * 70)
    ...
    def _v821_classified_execution_counts(self):
        """V8.2.5 authoritative intent-aware execution accounting.
        ...
        V8.2.5 uses the single V8 classifier so all reporting layers agree.
        """
        counts = {
        ...
        if counts["TOTAL"]["executed"] != len(executable):
            raise AssertionError(
                "V8.2.5 execution accounting mismatch: "
    ...
    def _v82_reconcile_execution_state(self):
        ...
        for item in results:
            ...
            # Use the same classifier as every other V8.2.5 truth layer.
            kind = self._v8_classify_test(item)
    ...
    def _v823_validate_coverage_truth(self):
        """Fail closed against impossible coverage claims."""
        truth = self._v823_behavioral_coverage_truth()

        if truth["behaviors_discovered"] == 0:
            assert truth["coverage_percent"] is None
            assert truth["status"] == "NOT_MEASURABLE"

        # V8.2.5: zero behaviors with discovered interactive surfaces is
        # not mission completion; it requires behavioral-model construction.
        interactive = 0
        ...
        if truth["behaviors_discovered"] == 0 and interactive > 0:
            truth["status"] = "MODEL_REQUIRED"
            truth["reason"] = "Interactive surfaces exist but no behavioral model was constructed."
        else:
            assert 0.0 <= truth["coverage_percent"] <= 100.0
            assert truth["behaviors_covered"] <= truth["behaviors_discovered"]

        print("\n📈 V8.2.5 COVERAGE TRUTH GATE: PASS")
        if truth["status"] == "NOT_MEASURABLE":
            print("   Behavioral coverage: NOT_MEASURABLE")
            print("   Behaviors discovered: 0")
        else:
            print(
                f"   Behavioral coverage: {truth['coverage_percent']:.2f}% "
                f"({truth['behaviors_covered']}/"
                f"{truth['behaviors_discovered']})"
            )
    ...
    def _v822_validate_execution_truth(self):
        ...
        if total["executed"] != total["passed"] + total["failed"]:
            raise AssertionError(
                "V8.2.5 invariant failed: executed != passed + failed"
            )
        ...
        for kind in ("SMOKE", "SANITY", "REGRESSION", "E2E"):
            ...
            if row["executed"] != row["passed"] + row["failed"]:
                raise AssertionError(
                    f"V8.2.5 invariant failed for {kind}: "
        ...
        if classified_generated != (
            ...
            raise AssertionError(
                "V8.2.5 generated-intent execution ledger mismatch"
            )

        print("\n🛡️ V8.2.5 EXECUTION TRUTH GATE: PASS")
        print(
    ...
    def _v825_validate_behavioral_mission(self):
        ...
        if truth.get("status") == "MODEL_REQUIRED":
            print("\n🧠 V8.2.5 BEHAVIOR MODEL GATE: MODEL_REQUIRED")
