    ...
    def generate_tests(self):
        ...
        for p in self.pages:
            ...
            for e in p["elements"]:
                ...
                # V8.6.1.1 hardens the final discovery → generation
                # contract. Every element reaching this loop is normalized
                # from directly observed DOM fields. Missing optional fields
                # are defaults; semantic is classified, never invented.
                if not isinstance(e, dict):
                    continue

                e.setdefault("visible", True)
                e.setdefault("aria_hidden", False)
                e.setdefault("disabled", False)
                e.setdefault("readonly", False)
                e.setdefault("required", False)
                e.setdefault("text", "")
                e.setdefault("aria_label", "")
                e.setdefault("placeholder", "")
                e.setdefault("name", "")
                e.setdefault("input_type", "")
                e.setdefault("role", "")
                e.setdefault("tag", "")
                ...
                if not semantic:
                    try:
                        semantic = _v85310_semantic_for_element(e)
                    except Exception:
                        semantic = "unknown"
                    e["semantic"] = semantic
...
# ============================================================================
# V8.6.1.1 — TRUE OBSERVABILITY / EVIDENCE CORRELATION
# ============================================================================
V852_VERSION = "8.5.2"
...
def _v85_write_truth(model, truth):
    d=Path("qa_v8_6_1_report"); d.mkdir(parents=True,exist_ok=True)
    mp=d/"qa_business_outcome_verification_v8_6_1.json"; tp=d/"qa_v8_6_1_business_outcome_truth.json"
    ...
    print("\n"+"="*70); print("🧭 V8.6.1.1 TRUE OBSERVABILITY / BUSINESS-OUTCOME TRUTH"); print("="*70)
    ...
    print(f"\n   {'🟢' if truth['pass'] else '🔴'} V8.6.1.1 EVIDENCE TRUTH GATE: {'PASS' if truth['pass'] else 'FAIL'}"); print("="*70); print(f"📄 Outcome verification: {mp}"); print(f"📄 Outcome truth: {tp}")


# ============================================================================
# V8.6.1.1 — FAIL-CLOSED AUTONOMOUS EXECUTION INTEGRITY
# ============================================================================
#
...
def _v853_write_integrity_artifact(payload):
    report_dir = Path("qa_v8_6_1_report")
    report_dir.mkdir(parents=True, exist_ok=True)
    path = report_dir / "qa_execution_integrity_v8_6_1.json"
    path.write_text(
...
def _v853_print_blocked_truth(payload, artifact):
    print("\n" + "=" * 70)
    print("🛡️ V8.6.1.1 FAIL-CLOSED EXECUTION INTEGRITY")
    print("=" * 70)
...
# ============================================================================
# V8.6.1.1 — TARGETED JOURNEY & UNKNOWN-BEHAVIOR CLOSURE
# ============================================================================
#
# Scope:
...
def _v8533_write_closure_artifact(summary):
    report_dir = Path("qa_v8_6_1_report")
    report_dir.mkdir(parents=True, exist_ok=True)
    path = report_dir / "qa_targeted_closure_v8_6_1.json"
    path.write_text(
...
async def _v8533_run_targeted_closure(agent):
    ...
    print("\n" + "=" * 70)
    print("🎯 V8.6.1.1 TARGETED JOURNEY & UNKNOWN-BEHAVIOR CLOSURE")
    print("=" * 70)
...
# ============================================================================
# V8.6.1.1 — FAILURE REPRODUCTION & ENVIRONMENT/APPLICATION CLASSIFICATION
# ============================================================================
#
...
def _v8534_write_reproduction_artifact(target, records, classification):
    ...
    path = report_dir / "qa_failure_reproduction_v8_6_1.json"
    path.write_text(
        json.dumps(payload, indent=2, ensure_ascii=False, default=str),
        encoding="utf-8",
    )
...
async def _v8534_failure_reproduction(agent, target):
    ...
    try:
        ...
        print("\n" + "=" * 70)
        print("🔬 V8.6.1.1 FAILURE REPRODUCTION")
        print("=" * 70)
...
# ============================================================================
# V8.6.1.1 — FAILURE RECORD NORMALIZATION / CRASH-PROOF REPORTING
# ============================================================================
...
# ============================================================================
# V8.6.1.1 — DETERMINISTIC FAILURE EVIDENCE & JOURNEY REPRODUCTION
# ============================================================================

def _v8536_classify_exception(exc):
    msg = str(exc or "")
    low = msg.lower()
    ...
    if "net::" in low or "connection" in low or "dns" in low:
        return "NETWORK_ERROR"
    if "assert" in low:
        return "ASSERTION_FAILURE"
    if "not found" in low or "locator" in low:
        return "ELEMENT_NOT_FOUND"
    if name in {"Error", "PlaywrightError"}:
        return "BROWSER_ERROR"
    return "UNCLASSIFIED_ERROR"


async def _v8536_capture_navigation(page, url, attempts=3, timeout_ms=60000):
    records = []
    ...
    for attempt in range(1, attempts + 1):
        ...
        try:
            ...
            if response is not None and 200 <= response.status < 400:
                record["status"] = "PASS"
                record["classification"] = "NAVIGATION_HEALTHY"
            else:
                record["status"] = "FAIL"
            ...
            record["classification"] = _v8536_classify_exception(exc)
            record["final_url"] = page.url
...
def _v8536_classify_records(records):
    ...
    if failures and not passes:
        ...
        return next(iter(classes), "NAVIGATION_FAILURE")
    if passes:
        return "NOT_REPRODUCED"
    return "NO_RESULT"


def _v8536_write_evidence(url, records, classification):
    report_dir = Path("qa_v8_6_1_report")
    report_dir.mkdir(parents=True, exist_ok=True)
    ...
    payload = {
        ...
        "classification": classification,
        "records": records,
        "confirmed_application_defect": False,
        "rule": (
            "A navigation failure cannot confirm an application defect. "
            "The affected behavior must execute and fail reproducibly."
        ),
    }

    path = report_dir / "qa_failure_evidence_v8_6_1.json"
    path.write_text(
        json.dumps(payload, indent=2, ensure_ascii=False, default=str),
        encoding="utf-8",
    )
...
async def _v8536_entrypoint_reproduction(agent, target):
    """Independently reproduce the entry-point failure before journey claims."""
    browser = getattr(agent, "browser", None)
    if browser is None:
        result = {
            "status": "FAIL",
        ...
        print("🔴 V8.6.1.1: browser handle unavailable")
        return result

    page = await browser.new_page()
    try:
        records = await _v8536_capture_navigation(page, target)
        classification = _v8536_classify_records(records)
        artifact = _v8536_write_evidence(
            target, records, classification
        )

        print("\n" + "=" * 70)
        print("🔬 V8.6.1.1 DETERMINISTIC FAILURE EVIDENCE")
        print("=" * 70)
        ...
        print(
            f"   PASS attempts  : "
            f"{sum(r.get('status') == 'PASS' for r in records)}"
        )
        print(
            f"   FAIL attempts  : "
            f"{sum(r.get('status') == 'FAIL' for r in records)}"
        )
...
# ============================================================================
# V8.6.1.1 — CURRENT-RUN DISCOVERY / EXECUTION BARRIER
# ============================================================================
#
# HARD INVARIANT:
#     current_pages == 0  =>  test_execution == 0
#
# No legacy tests, journeys, behavioral models, or prior-run execution
# records may execute when current-run discovery has not established a model.
# ============================================================================


def _v8538_current_model_is_valid(pages, target):
    if not isinstance(pages, list) or not pages:
        return False, "CURRENT_MODEL_EMPTY"

    normalized = {
        str(p.get("url", p) if isinstance(p, dict) else p).rstrip("/")
        for p in pages
    }
    target_norm = str(target).rstrip("/")

    if target_norm not in normalized and "https://demoqa.com" not in normalized:
        return False, "TARGET_NOT_IN_CURRENT_MODEL"

    return True, "CURRENT_MODEL_VALID"


async def _v8538_direct_current_discovery(target, headless=True):
    """
    Independent current-run discovery. Does not use old application_map,
    journeys, behavioral models, or legacy execution records.
    """
    from playwright.async_api import async_playwright
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if response is None or not (200 <= response.status < 400):
                return {
                    "ok": False,
                    "reason": "TARGET_HTTP_FAILURE",
                    "pages": [],
                    "http_status": response.status if response else None,
                }

            async def add_current_page():
                url = page.url.rstrip("/")
                ...
                # Preserve the schema expected by the existing behavior model:
                # `elements` is iterable. Keep the count separately.
            ...
            await add_current_page()

            # Discover same-origin links from the live DOM. Limit depth/work
            # so discovery is deterministic and cannot become an open crawler.
            for _depth in range(2):
                urls = await page.locator("a[href]").evaluate_all(
                ...
                for u in urls:
                    u = str(u).split("#")[0].rstrip("/")
                    if u.startswith(origin) and u not in seen:
                        candidates.append(u)
                ...
                for u in candidates[:100]:
                    try:
                        ...
                        if r is None or not (200 <= r.status < 400):
                            continue
                        await add_current_page()
                    except Exception:
                        continue

                # Return to root before collecting another live link set.
                try:
                    await page.goto(
                        target,
                        wait_until="domcontentloaded",
            ...
            ok, reason = _v8538_current_model_is_valid(pages, target)
            return {
                "ok": ok,
                "reason": reason,
                "pages": pages,
...
def _v8538_clear_legacy_execution_state(agent):
    blocked = []
    containers = (
        "tests", "test_cases", "test_results", "results",
        "execution_results", "execution_records", "journeys",
        "business_journeys", "behavioral_model", "behavior_model",
    )
    for attr in containers:
...
def _v8538_write_integrity_artifact(
    target, discovery_ok, reason, pages, execution_started
):
    report_dir = Path("qa_v8_6_1_report")
    report_dir.mkdir(parents=True, exist_ok=True)
    payload = {
        ...
        "current_pages": len(pages) if isinstance(pages, list) else 0,
        "execution_started": execution_started,
        "hard_invariant": "current_pages == 0 => execution == 0",
        "stale_state_reuse": False,
        "synthetic_execution": False,
        "fail_closed": not discovery_ok,
        "pages": pages if isinstance(pages, list) else [],
    }
    path = report_dir / "qa_execution_barrier_v8_6_1.json"
    path.write_text(
        json.dumps(payload, indent=2, ensure_ascii=False, default=str),
        encoding="utf-8",
    )
...
async def _v8538_execution_barrier(agent, target, headless=True):
    result = await _v8538_direct_current_discovery(target, headless)
    pages = result.get("pages", [])

    if not result.get("ok"):
        blocked = _v8538_clear_legacy_execution_state(agent)
        artifact = _v8538_write_integrity_artifact(
            target, False, result.get("reason", "DISCOVERY_FAILED"),
            [], False
        )
        print("\n" + "=" * 70)
        print("🛡️ V8.6.1.1 CURRENT-RUN DISCOVERY / EXECUTION BARRIER")
        print("=" * 70)
        print("   Discovery status : FAIL")
        print(f"   Reason           : {result.get('reason', 'DISCOVERY_FAILED')}")
        print("   Current pages    : 0")
        print("   Tests executed   : 0")
        print(f"   Legacy state     : BLOCKED ({len(blocked)} containers)")
        print("   Execution        : NOT STARTED")
        print("   🔴 FAIL-CLOSED: no current application model.")
        print(f"📄 Integrity artifact: {artifact}")
        return False

    # Current live model is now established. Make it available to the agent.
    try:
        agent.pages = pages
    except Exception:
        pass
    ...
    artifact = _v8538_write_integrity_artifact(
        target, True, result.get("reason", "CURRENT_MODEL_VALID"),
        pages, False
    )
    ...
    print("🛡️ V8.6.1.1 CURRENT-RUN DISCOVERY / EXECUTION BARRIER")
    print("=" * 70)
    print("   Discovery status : PASS")
    print(f"   Current pages    : {len(pages)}")
    print("   Current model    : LIVE DOM")
...
# ============================================================================
# V8.6.1.1 — DISCOVERY SCHEMA CONTRACT / BEHAVIOR-MODEL TYPE GUARD
# ============================================================================

def _v8539_normalize_current_pages(pages):
    normalized = []
    ...
    for page in pages or []:
        ...
        # Old/current adapters may provide only a numeric element count.
        # Never allow an integer to reach `for e in page["elements"]`.
        if isinstance(elements, int):
            item["element_count"] = elements
            item["elements"] = []
        elif elements is None:
            item["elements"] = []
        elif not isinstance(elements, list):
            try:
                item["elements"] = list(elements)
            except TypeError:
                item["elements"] = []

        item.setdefault("element_count", len(item["elements"]))
        item["current_run"] = True
        normalized.append(item)

    return normalized


def _v8539_harden_behavior_model_builder(agent):
    """
    Wrap the existing V8.2 behavior-model builder so malformed element
    containers cannot crash the run. This does not turn missing elements into
    discovered behavior; it records an empty element set honestly.
    """
    original = getattr(agent, "_v826_build_behavior_model", None)
    if original is None or getattr(original, "_v8539_hardened", False):
        return

    async def wrapped_async(*args, **kwargs):
        if isinstance(getattr(agent, "pages", None), list):
            agent.pages = _v8539_normalize_current_pages(agent.pages)
        try:
            ...
            print(
                f"🔴 V8.6.1.1 BEHAVIOR-MODEL TYPE ERROR CAPTURED: "
                f"{type(exc).__name__}: {exc}"
            )
            raise
    ...
    if inspect.iscoroutinefunction(original):
        wrapped_async._v8539_hardened = True
        agent._v826_build_behavior_model = wrapped_async
    else:
        wrapped_sync._v8539_hardened = True
        agent._v826_build_behavior_model = wrapped_sync



# ============================================================================
# V8.6.1.1 — DISCOVERY → TEST-GENERATION SCHEMA ADAPTER
# ============================================================================

def _v85310_semantic_for_element(element):
    """Derive conservative UI semantics from directly observed DOM evidence."""
    ...
    aria = str(element.get("aria_label", "")).lower()
    ...
    if tag == "input":
        ...
        return input_map.get(typ, "text_input")
    if tag == "a":
        return "link"

    # Conservative semantic hints only when directly observable.
    combined = " ".join((text, name, aria))
...
def _v85310_enrich_current_pages(pages):
    """Add required semantic fields without fabricating unobserved behavior."""
    enriched = []
    ...
    for page in pages or []:
        ...
        for element in elements:
            if not isinstance(element, dict):
                continue
            e = dict(element)
            e.setdefault("semantic", _v85310_semantic_for_element(e))
            e.setdefault("observed_current_run", True)
            normalized_elements.append(e)

        item["elements"] = normalized_elements
        item["element_count"] = len(normalized_elements)
        item["current_run"] = True
        enriched.append(item)
...
def _v85310_harden_test_generator(agent):
    """
    Normalize the current-run behavior model immediately before legacy
    generate_tests() consumes it. Missing semantic values become 'unknown',
    never an invented business action.
    """
    original = getattr(agent, "generate_tests", None)
    if original is None or getattr(original, "_v85310_hardened", False):
        return

    def normalize_model():
        # Normalize common model containers if present.
        for attr in ("pages", "application_model", "behavior_model"):
            value = getattr(agent, attr, None)
            if isinstance(value, list):
                setattr(agent, attr, _v85310_enrich_current_pages(value))

        model = getattr(agent, "behavior_model", None)
            ...
            for page in model.get("pages", []) or []:
                ...
                for e in elems:
                    if isinstance(e, dict):
                        e.setdefault(
                            "semantic",
                            _v85310_semantic_for_element(e)
                        )
    ...
    wrapped._v85310_hardened = True
    agent.generate_tests = wrapped



# ============================================================================
# V8.6.1.1 — CANONICAL AUTONOMOUS QA ARCHITECTURE
# ============================================================================
#
# Stage contract:
# LIVE DISCOVERY -> CANONICAL MODEL -> BEHAVIORS -> JOURNEYS
# -> TEST INTENTS -> EXECUTION -> EVIDENCE -> TRUTH
#
# No legacy state is allowed to become current-run evidence.
# ============================================================================

V86_SCHEMA_VERSION = "8.6.1"
V86_STAGE_ORDER = (
    "DISCOVERY",
    "MODEL",
    "BEHAVIOR",
...
def _v86_canonical_element(e):
    ...
    return {
        ...
        "id": str(e.get("id", "")),
        "name": str(e.get("name", "")),
        "type": str(e.get("type", e.get("input_type", ""))),
        "role": str(e.get("role", "")),
        "text": str(e.get("text", ""))[:500],
        "href": str(e.get("href", "")),
        "placeholder": str(e.get("placeholder", "")),
        "aria_label": str(e.get("aria_label", "")),
        "semantic": semantic,
        "observed_current_run": True,
    }


def _v86_canonical_page(page):
    if not isinstance(page, dict):
        return None
    ...
    canonical_elements = []
    for e in elements:
        ce = _v86_canonical_element(e)
        if ce is not None:
            canonical_elements.append(ce)

    url = str(page.get("url", "")).split("#")[0].rstrip("/")
    if not url:
        return None

    return {
        "schema_version": V86_SCHEMA_VERSION,
        "url": url,
...
def _v86_build_canonical_model(pages, target):
    canonical = []
    seen = set()

    for page in pages or []:
        cp = _v86_canonical_page(page)
        if cp is None or cp["url"] in seen:
            continue
        seen.add(cp["url"])
        canonical.append(cp)
    ...
    target_norm = str(target).rstrip("/")
    target_present = any(
        p["url"] == target_norm or p["url"] == "https://demoqa.com"
        for p in canonical
    )
    ...
    return {
        ...
        "valid": bool(canonical and target_present),
    }


def _v86_validate_model(model):
    errors = []

    if not isinstance(model, dict):
        return ["MODEL_NOT_OBJECT"]

    if model.get("schema_version") != V86_SCHEMA_VERSION:
        errors.append("MODEL_SCHEMA_VERSION_INVALID")
    if model.get("current_run") is not True:
        errors.append("MODEL_NOT_CURRENT_RUN")
    if not isinstance(model.get("pages"), list) or not model["pages"]:
        errors.append("MODEL_HAS_NO_PAGES")

    for i, page in enumerate(model.get("pages", [])):
        if not isinstance(page, dict):
            errors.append(f"PAGE_{i}_NOT_OBJECT")
            continue
        if not page.get("current_run"):
            errors.append(f"PAGE_{i}_NOT_CURRENT_RUN")
        if not isinstance(page.get("elements"), list):
            errors.append(f"PAGE_{i}_ELEMENTS_NOT_LIST")
        for j, e in enumerate(page.get("elements", [])):
            if not isinstance(e, dict):
                errors.append(f"PAGE_{i}_ELEMENT_{j}_NOT_OBJECT")
                continue
            if "semantic" not in e:
                errors.append(f"PAGE_{i}_ELEMENT_{j}_MISSING_SEMANTIC")

    return errors


def _v86_install_canonical_model(agent, pages, target):
    model = _v86_build_canonical_model(pages, target)
    errors = _v86_validate_model(model)

    if errors:
        raise RuntimeError(
            "V8.6.1.1 MODEL CONTRACT FAILED: " + ", ".join(errors[:20])
        )

    # Keep both a canonical model and a compatibility view for legacy methods.
    # The compatibility view is generated ONLY from the current canonical model.
    agent.v86_model = model
    agent.pages = model["pages"]
    agent.v86_stage = "MODEL"
    return model


def _v86_validate_stage(stage, agent):
    if stage not in V86_STAGE_ORDER:
        raise RuntimeError(f"V8.6.1.1 UNKNOWN_STAGE: {stage}")

    if stage != "DISCOVERY":
        model = getattr(agent, "v86_model", None)
        errors = _v86_validate_model(model)
        if errors:
            raise RuntimeError(
                f"V8.6.1.1 {stage} BLOCKED: canonical model invalid: "
                + ", ".join(errors[:10])
            )

    agent.v86_stage = stage
    return True


def _v86_write_contract_artifact(agent, status, reason="", extra=None):
    report_dir = Path("qa_v8_6_1_report")
    report_dir.mkdir(parents=True, exist_ok=True)

    payload = {
        "version": V86_SCHEMA_VERSION,
        "stage": getattr(agent, "v86_stage", "UNKNOWN"),
        "status": status,
        "reason": reason,
        "current_run_only": True,
        "stale_state_reuse": False,
        ...
        "model_pages": (
            getattr(getattr(agent, "v86_model", None), "page_count", None)
            if False else
            (
                getattr(agent, "v86_model", {}).get("page_count", 0)
                if isinstance(getattr(agent, "v86_model", None), dict) else 0
            )
        ),
    }
    ...
    path = report_dir / "qa_v8_6_1_stage_contract.json"
    path.write_text(
        json.dumps(payload, indent=2, ensure_ascii=False, default=str),
        encoding="utf-8",
    )
...
def _v86_patch_run(agent):
    """
    Make every legacy stage consume the same canonical model.
    This is deliberately a compatibility layer, not a second model.
    """
    original_build = getattr(agent, "_v826_build_behavior_model", None)
    if original_build and not getattr(original_build, "_v86_wrapped", False):

        def build_wrapper(*args, **kwargs):
            _v86_validate_stage("BEHAVIOR", agent)
            return original_build(*args, **kwargs)

        build_wrapper._v86_wrapped = True
        agent._v826_build_behavior_model = build_wrapper

    original_generate = getattr(agent, "generate_tests", None)
    if original_generate and not getattr(original_generate, "_v86_wrapped", False):

        def generate_wrapper(*args, **kwargs):
            _v86_validate_stage("TEST_INTENT", agent)
            return original_generate(*args, **kwargs)

        generate_wrapper._v86_wrapped = True
        agent.generate_tests = generate_wrapper


async def _v86_run_canonical_pipeline(agent, target, headless=True):
    """
    V8.6.1.1 replaces the fragile internal-discovery dependency with direct
    current-run discovery, then establishes a canonical model before any
    behavior/test-generation stage is permitted.
    """
    result = await _v8538_direct_current_discovery(target, headless)
    pages = result.get("pages", [])

    if not result.get("ok"):
        _v8538_clear_legacy_execution_state(agent)
        path = _v86_write_contract_artifact(
            agent,
            "BLOCKED",
            result.get("reason", "DISCOVERY_FAILED"),
            {"current_pages": 0, "execution_authorized": False},
        )
        print("\n" + "=" * 70)
        print("🛡️ V8.6.1.1 CANONICAL DISCOVERY GATE")
        print("=" * 70)
    ...
    model = _v86_install_canonical_model(agent, pages, target)
    _v86_patch_run(agent)
    _v86_validate_stage("MODEL", agent)
    ...
    print("   Model     : CANONICAL / CURRENT-RUN")
    print("   Execution : AUTHORIZED")
    print("=" * 70)

    _v86_write_contract_artifact(
        agent,
        "PASS",
        "CURRENT_CANONICAL_MODEL_ESTABLISHED",
        {
            "current_pages": model["page_count"],
...
# ============================================================================
# V8.6.1 — CANONICAL STATE ISOLATION + EVIDENCE CLOSURE
# ============================================================================

V861_VERSION = "8.6.1"
V861_REPORT_DIR = Path("qa_v8_6_1_report")
V861_LEGACY_PREFIXES = (
    "qa_v8_5_report",
    "qa_v8_5_3_",
    "qa_v8_2_report",
)
...
def _v861_report_path(name):
    V861_REPORT_DIR.mkdir(parents=True, exist_ok=True)
    return V861_REPORT_DIR / name


def _v861_assert_no_legacy_release_inputs(target):
    """
    V8.6.1 release truth is generated only from current-run V8.6.1 objects.
    Legacy artifacts may exist on disk but cannot be loaded as release inputs.
    """
    forbidden = []
    for candidate in (
        getattr(target, "run_state", None),
        getattr(target, "canonical_truth", None),
        getattr(target, "application_map", None),
        getattr(target, "test_report", None),
    ):
        if candidate is None:
            continue
        value = str(candidate)
        if any(prefix in value for prefix in V861_LEGACY_PREFIXES):
            forbidden.append(value)

    if forbidden:
        raise RuntimeError(
            "V8.6.1 LEGACY_STATE_CONTAMINATION: " + ", ".join(forbidden)
        )


def _v861_fresh_containers(agent):
    """
    Clear mutable execution containers before current-run work. This does not
    delete files and does not fabricate results.
    """
    names = (
        "results",
        "test_results",
...
def _v861_current_run_id():
    import uuid
    return uuid.uuid4().hex


def _v861_evidence_record(execution_id, kind, observed, source):
    return {
        "version": V861_VERSION,
...
def _v861_classify_outcome(evidence):
    """
    Strict evidence classification:
      OBSERVED  -> directly evidenced by current run
      INFERRED  -> reasoned from observed UI/state/network evidence
    ...
    if any(
        isinstance(e, dict)
        and e.get("observed") is True
        and e.get("current_run") is True
        and e.get("synthetic") is False
        for e in evidence
    ):
        return "OBSERVED"
...
def _v861_release_truth(agent):
    """
    Fail closed. This function does not manufacture coverage, journey passes,
    backend verification, or business outcomes.
    """
    model = getattr(agent, "v86_model", None)
    if not isinstance(model, dict):
        return {
    ...
    unknown = getattr(agent, "unknown_behaviors", [])
    if not isinstance(unknown, list):
        unknown = []

    journey_failures = []
    journeys = getattr(agent, "journeys", [])
    if isinstance(journeys, list):
        journey_failures = [
            j for j in journeys
            if isinstance(j, dict)
            and str(j.get("status", "")).upper() in {"FAIL", "FAILED"}
        ]

    evidence = getattr(agent, "evidence", [])
    if not isinstance(evidence, list):
        evidence = []

    synthetic = [
        e for e in evidence
        if isinstance(e, dict) and e.get("synthetic") is True
    ]
    ...
    result = {
        ...
        "synthetic_evidence": len(synthetic),
        "pages": page_count,
        "unknown_behaviors": len(unknown),
        "journey_failures": len(journey_failures),
        "evidence_records": len(evidence),
        "release": "PASS",
        "gates": {
            "canonical_model": page_count > 0,
            "legacy_isolation": True,
            "synthetic_evidence": len(synthetic) == 0,
            "unknown_behavior": len(unknown) == 0,
            "journeys": len(journey_failures) == 0,
        },
    }
...
def _v861_write_truth(agent, reason=""):
    payload = _v861_release_truth(agent)
    payload["reason"] = reason

    path = _v861_report_path("qa_v8_6_1_canonical_truth.json")
    path.write_text(
        json.dumps(payload, indent=2, ensure_ascii=False, default=str),
        encoding="utf-8",
    )
...
def _v861_install(agent):
    run_id = _v861_current_run_id()
    agent.v861_run_id = run_id
    agent.v861_version = V861_VERSION
    agent.v861_stage = "INITIALIZED"

    cleared = _v861_fresh_containers(agent)
    agent.v861_cleared_containers = cleared

    # Prevent accidental legacy release paths from being used.
    for attr in (
        "run_state",
        "canonical_truth",
        "application_map",
        ...
        if hasattr(agent, attr):
            ...
            if value is not None and any(
                prefix in str(value) for prefix in V861_LEGACY_PREFIXES
            ):
...
def _v861_assert_execution_integrity(agent):
    if not getattr(agent, "v861_run_id", None):
        raise RuntimeError("V8.6.1 RUN_ID_MISSING")

    if getattr(agent, "v86_model", None) is None:
        raise RuntimeError("V8.6.1 EXECUTION_BLOCKED_NO_CANONICAL_MODEL")

    model = agent.v86_model
    if model.get("current_run") is not True:
        raise RuntimeError("V8.6.1 EXECUTION_BLOCKED_NON_CURRENT_MODEL")

    if not model.get("valid"):
        raise RuntimeError("V8.6.1 EXECUTION_BLOCKED_INVALID_MODEL")
...
async def main():
    ...
    print("\n*** RUNNING qa_agent_v8_6_1_FAIL_CLOSED_EXECUTION_INTEGRITY.py — VERSION 8.6.1 ***\n")

    if not url.startswith(("http://", "https://")):
        print("❌ Invalid URL")
        sys.exit(1)

    # CRITICAL V8.6.1.1 RULE:
    ...
    print("🛡️ V8.6.1.1 CURRENT-RUN DISCOVERY PREFLIGHT")
    ...
    if not _v853_connectivity_gate(preflight, target):
        ...
        print("🔴 V8.6.1.1 STOP: agent execution was NOT started.")
        raise SystemExit(3)
    ...
    barrier_ok = await _v8538_execution_barrier(agent, target, getattr(args, "headless", True))
    if not barrier_ok:
        return
    await agent.run()
