...
# ============================================================
# AUTONOMOUS QA AGENT V4 — Goal-Driven Autonomous QA
...
class QAAgent:
    ...
    # ========================================================
    # V4 GOAL-DRIVEN QUALITY PLANNING
    # ========================================================

    def risk_score(self, test):
        """
        Estimate risk from observable application semantics.

        This is deliberately deterministic: the agent does not invent
        business facts that were not observed in the application.
        """
        ...
        if semantic == "button":
            score += 20
            reasons.append("action control")

        # Dynamic behavior is inherently higher risk.
        if (
            "dynamic" in description
            or "wait" in description
            or "enable" in description
        ):
            score += 20
            reasons.append("dynamic behavior")

        # Dialog/alert flows can have side effects.
        if (
            "alert" in url
            or "dialog" in description
        ...
        # Forms deserve additional attention.
        if (
            "form" in url
            or semantic in {
                "text_input",
            ...
            score += 10
            reasons.append("form/input path")

        # Navigation and page-load tests are useful baseline tests,
        # but normally lower risk than state-changing interactions.
    ...
    def build_quality_plan(self):
        """
        Convert generated tests into an explicit risk-driven execution plan.
        """
        ...
        for index, test in enumerate(self.tests):
            score, level, reasons = self.risk_score(test)
        ...
        # High-risk tests first; preserve original ordering for ties.
        planned.sort(
            key=lambda x: (
                -x["plan"]["risk_score"],
                x["plan"]["original_index"],
            )
        )
        ...
        print(
            f"   HIGH   : {self.stats['risk_high']}"
        )
        print(
            f"   MEDIUM : {self.stats['risk_medium']}"
        )
        print(
            f"   LOW    : {self.stats['risk_low']}"
        )
        ...
        for test in self.tests[:10]:
            ...
            print(
                f"   #{plan['priority']:02d} "
                f"[{plan['risk_level']:<6}] "
                f"{plan['risk_score']:02d} "
                f"{clean(test.get('description'))[:90]}"
            )

    # ========================================================
    # V4 ADAPTIVE RETEST PLANNING
    # ========================================================

    def build_retest_plan(self):
        """
        Convert failures into a deterministic retest queue.

        The current V4 implementation records the retest intent without
        automatically duplicating execution, preventing infinite loops.
        """
    ...
    async def run(self):
        ...
        retests = self.build_retest_plan()

        if retests:
            print(
                f"\n🔄 RETEST CANDIDATES: {len(retests)}"
            )
            for item in retests[:10]:
                print(
                    f"   • {item['description']} "
                    f"→ {item['reason']}"
                )
    ...
    def report(self):
        ...
        print(
            "📊 V4.0 GOAL-DRIVEN AUTONOMOUS QA REPORT"
        )
        ...
        print(
            f"Risk HIGH        : "
            f"{self.stats['risk_high']}"
        )

        print(
            f"Risk MEDIUM      : "
            f"{self.stats['risk_medium']}"
        )

        print(
            f"Risk LOW         : "
            f"{self.stats['risk_low']}"
        )
...
async def main():
    print("\n*** RUNNING qa_agent_v4_FINAL.py — VERSION 4.0 FINAL ***\n")
    print("\n*** RUNNING qa_agent_v4_FINAL.py — VERSION 4.0 FINAL ***\n")
    ...
    if len(sys.argv) != 2:
        ...
        print(
            'Usage: python3 qa_agent_v4_FINAL.py '
            '"https://demoqa.com"'
        )
