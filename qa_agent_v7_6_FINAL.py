...
REPORT_DIR = Path("qa_v7_5_report")
...
""".format(
    ...
    def _v76_diff(self, old, new):
        ou = set(old.get("urls", {})); nu = set(new.get("urls", {}))
        ...
        for url in sorted(ou & nu):
            oe = old["urls"][url].get("elements", {}); ne = new["urls"][url].get("elements", {})
            ...
            if (old["urls"][url].get("fingerprint") != new["urls"][url].get("fingerprint")
                    or old["urls"][url].get("title") != new["urls"][url].get("title")):
                changed.append(url)
        return {
    ...
    def _v76_print(self):
        ...
        print("\n" + "=" * 70)
        print("🔄 V7.6 CHANGE-AWARE QA INTELLIGENCE")
        print("=" * 70)
    ...
    def _v76_write_artifact(self):
        try:
            (REPORT_DIR / "qa_change_intelligence_v7_6.json").write_text(
                json.dumps({
                    "version": "7.6",
                    "change_risk": getattr(self, "v76_change_risk", 0),
            ...
            print(f"\n📄 Change-intelligence artifact: {(REPORT_DIR / 'qa_change_intelligence_v7_6.json').absolute()}")
        except Exception as exc:
    ...
    def report(self):
        ...
        print(
            "📊 V7.6 CHANGE-AWARE AUTONOMOUS QA REPORT"
        )
