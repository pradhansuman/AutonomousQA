#!/usr/bin/env python3
"""
V8.8 - DISCOVERY ROBUSTNESS + SEMANTIC MODEL FIX
...
VERSION = "8.8"
REPORT_DIR = Path("qa_v8_8_report")
...
class Agent:
    ...
    async def navigate(self,page,url):
        """Cross-site, fail-soft current-run navigation.

        Navigation readiness must not manufacture a behavior failure merely
        because a generic body attachment check is slow or unstable.

        A usable document is established from current-run browser state only.
        Semantic execution remains responsible for proving the target control.
        """
        ...
        try:
            ...
            self.log(
                f"⚠️ NAVIGATION EXCEPTION | "
                f"{type(exc).__name__}: {exc}"
            )

        # Bounded document readiness. Never require networkidle.
        try:
        ...
        # Verify that a browser document currently exists. This is deliberately
        # fail-soft: the semantic locator is the authoritative readiness check
        # for the actual behavior under test.
        document_ready = False
        try:
            document_ready = await page.evaluate(
        ...
        if not document_ready:
            # One bounded current-run recovery attempt.
            self.log(f"🔄 DOCUMENT RECOVERY RELOAD | {url}")
            try:
        ...
        if not document_ready:
            raise RuntimeError(
                f"no usable current-run document after navigation: {url}"
            )

        # Optional bounded application-content observation. Failure here does
        # not invalidate the document; the target semantic locator may still
        # be fully usable.
        deadline = time.monotonic() + max(self.dom_timeout, 5000) / 1000
        while time.monotonic() < deadline:
            try:
                ...
                if links > 0 or len((body_text or "").strip()) >= 20:
                    break
            except Exception:
                pass
    ...
    async def discover(self,browser):
        self.log("="*70)
        self.log("🗺️ V8.8 CURRENT-RUN DISCOVERY")
    ...
    def build_behaviors(self):
        ...
        """
        self.log("="*70)
        self.log("🧠 V8.8 CANONICAL SEMANTIC BEHAVIOR MODEL")
    ...
    async def execute_safe_navigation(self,browser):
        ...
        V8.8 uses the Element attributes directly and deduplicates by
        canonical destination surface.
        """
        self.log("="*70)
        self.log("🧪 V8.8 SAFE NAVIGATION EXECUTION")
    ...
    async def _resolve_locator(self, page, semantic, selector, label):
        """V8.8 current-DOM semantic resolver with bounded readiness/fallbacks."""
        ...
        elif s in ("TEXT_INPUT", "NUMBER_INPUT", "TEXTAREA"):
            # Prefer stable IDs when the discovered selector is an ID.
            # DemoQA's React forms can expose the label only after the input
            # has mounted, so the ID fallback must remain current-DOM based.
            if selector and selector.startswith("#") and len(selector) > 1:
                ident = selector[1:]
                add("input-id", page.locator(
                    f"#{ident}:visible:not([type='hidden']):not([disabled])"
                ).first)
    ...
    async def _execute_one_semantic(self, page, loc, semantic, label):
        """V8.8 execute semantics and observe actual current UI state."""
        s = str(semantic or "").upper()
        detail = {"semantic": s, "label": label}

        if s == "FILE_UPLOAD":
            # Playwright file inputs expose the selected file through the
            # input value/files property; HTML attributes are not a reliable
            # observation after set_input_files().
            probe = Path("/tmp/qa_v8_7_20_upload_probe.txt")
            probe.write_text(
                "V8.8 current-run upload probe\n",
                encoding="utf-8"
            )
            await loc.set_input_files(str(probe))

            observed_value = await loc.input_value()
            files = await loc.evaluate(
                """e => Array.from(e.files || []).map(f => ({
                    name: f.name,
                    size: f.size,
                    type: f.type
                }))"""
            ...
            if files[0].get("name") != probe.name:
                raise RuntimeError(
                    f"file upload observation mismatch: "
                    f"expected={probe.name!r} observed={files!r}"
                )

            detail.update(
                observed_file_value=observed_value,
                observed_files=files,
                upload_verified=True
            )
            return detail

        if s in ("TEXT_INPUT", "NUMBER_INPUT", "TEXTAREA"):
            # Runtime semantic correction: a discovered range input must never
            # be treated as ordinary text merely because discovery called it
            # TEXT_INPUT.
            input_type = (
            ...
            if input_type == "range" or role == "slider":
            ...
            if "datePickerMonthYearInput" in element_id:
                probe = "08/31/2026"
            ...
            if s == "TEXTAREA":
            ...
            elif "datePickerMonthYearInput" in element_id:
                if not observed:
                    raise RuntimeError("date input observation empty after fill")
            else:
                maxlength_raw = await loc.get_attribute("maxlength")
                maxlength = None
                try:
                    if maxlength_raw is not None:
                        maxlength = int(maxlength_raw)
                except (TypeError, ValueError):
                    pass

                expected = probe[:maxlength] if maxlength is not None and maxlength >= 0 else probe
                ...
                detail["input_constraint"] = {
                    "maxlength": maxlength,
                    "expected_after_constraint": expected
                }
        ...
        if s == "SLIDER":
            ...
            after = await loc.input_value()
            if before == after:
                # Second bounded attempt: move in the opposite direction.
                await loc.press("ArrowLeft")
                after = await loc.input_value()

            if before == after:
                # Boundary-safe attempt. Home should move a range to its
                # minimum when ArrowLeft was already at the minimum.
                await loc.press("Home")
                after = await loc.input_value()

            if before == after:
                # Final bounded boundary attempt.
                await loc.press("End")
                after = await loc.input_value()
    ...
    async def execute_semantic_behaviors(self, browser):
        """V8.8 targeted semantic execution with bounded recovery and runtime semantic correction."""
        self.log("="*70)
        self.log("🧪 V8.8 TARGETED SEMANTIC EXECUTION")
        ...
        if unresolved:
            self.log(
                f"🔁 V8.8 CLOSURE PASS | unresolved={len(unresolved)}"
            )
        for b in unresolved:
            page = None
            semantic = str(b.get("semantic", "")).upper()
        ...
        self.log(
            f"🧪 V8.8 SEMANTIC EXECUTION COMPLETE | "
    ...
    def truth(self,discovery_ok):
        ...
        self.log("="*70)
        self.log("🧭 V8.8 SINGLE CANONICAL TRUTH")
        self.log("="*70)
        for k,v in truth.items():
            if k=="gates": continue
            self.log(f"{k.replace('_',' ').title():30}: {v}")
        self.log("-"*70)
        self.log("🛡️ V8.8 TRUTH GATES")
        self.log("-"*70)
        for k,v in gates.items():
            self.log(f"{k:30} : {'PASS' if v else 'FAIL'}")
        self.log("-"*70)
        self.log(
            "🟢 V8.8 RELEASE TRUTH GATE: PASS"
            if release else
            "🔴 V8.8 RELEASE TRUTH GATE: FAIL\n"
    ...
    def write(self,truth):
        ...
        (REPORT_DIR/"qa_v8_8_canonical_truth.json").write_text(
    ...
    async def run(self):
        self.log("="*70)
        self.log("🚀 V8.8 DISCOVERY ROBUSTNESS + SEMANTIC QA")
        self.log("="*70)
        ...
        async with async_playwright() as p:
            ...
            self.write(truth)
            await browser.close()
            self.log(f"📄 Canonical truth: {REPORT_DIR/'qa_v8_8_canonical_truth.json'}")
