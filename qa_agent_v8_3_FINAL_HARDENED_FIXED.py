...
REPORT_DIR = Path("qa_v8_2_report")
...
# ============================================================
# V8.2.11 INTEGRATED UNKNOWN-BEHAVIOR CLOSURE
# ============================================================
    ...
    async with async_playwright() as p:
        ...
        for behavior in unknown[:V828_MAX_CLOSURE_ROUNDS]:
            ...
            if result.get("status") == "PASS":
                ...
                self.results.append({
                    "name": f"V8.2.11 Closure: {behavior.get('description', behavior.get('action'))}",
...
def _v828_write_closure_artifact(self, closure):
    path = REPORT_DIR / "qa_unknown_behavior_closure_v8_2_11.json"
    payload = {
        "version": "8.2.8",
    ...
    print(f"\n📄 V8.2.11 closure artifact: {path.absolute()}")
...
# ============================================================
# V8.2.11 EXECUTION PROVENANCE & EVIDENCE INTEGRITY
# ============================================================
...
def _v829_write_provenance_artifact(report_dir, results):
    ...
    path = Path(report_dir) / "qa_execution_provenance_v8_2_11.json"
    path.write_text(
...
def _v829_print_provenance(report):
    print("\n" + "=" * 70)
    print("🔎 V8.2.11 EXECUTION PROVENANCE & EVIDENCE INTEGRITY")
    print("=" * 70)
...
def _v8210_write_truth_artifact(report_dir, truth):
    path = Path(report_dir) / "qa_truth_gate_v8_2_11.json"
    path.write_text(
...
def _v8210_print_truth_gate(truth):
    ...
    if truth["truth_gate"] == "PASS":
        print("\n   🟢 V8.2.11 TRUTH GATE: PASS")
    else:
        print("\n   🔴 V8.2.11 TRUTH GATE: FAIL")
...
V8211_VERSION = "8.2.11"

# V8.2.11 PATCH: authoritative version + risk-based evidence truth.
V8211_PATCH_LEVEL = "8.2.11-final | V8.3-business-journeys"


def _v8211_version_from_text(value):
    """
    Extract an authoritative V8.2.11 declaration.
...
def _v8211_write_truth_artifact(report_dir, truth):
    path = Path(report_dir) / "qa_truth_single_source_v8_2_11.json"
    path.write_text(
        json.dumps(truth, indent=2, ensure_ascii=False, default=str),
        encoding="utf-8",
    )
...
def _v83_release_truth(discovery=None, execution=None):
...
async def _v83_execute_business_journeys(agent):
    ...
    async with async_playwright() as p:
        ...
        for journey in sorted(
            ...
            for surface in journey.get("steps", [])[:V83_MAX_STEPS]:
                step_result = await _v83_execute_safe_surface(page, surface)
...
class QAAgent:
    async def _v83_run_business_journey_intelligence(self):
        ...
        discovery, execution = _v83_write_business_artifacts(
            self,
            graph,
            journeys,
            executions,
        ...
        release_truth = _v83_release_truth(discovery, execution)
    ...
    async def _v828_run_unknown_behavior_closure(self):
        ...
        print("\n" + "=" * 70)
        print("🧠 V8.2.11 UNKNOWN-BEHAVIOR CLOSURE")
        print("=" * 70)
...
""".format(
    ...
    def _v70_print_mission(self):
        ...
        print("\n")
        print("=" * 70)
        print("🎯 V8.2.6 AUTONOMOUS QA MISSION")
        print("=" * 70)
    ...
    def _v824_mission_truth(self):
        ...
        V8.2.6 could print 100% while reporting zero behaviors. V8.2.6
    ...
    def _v76_print(self):
        ...
        print("\n" + "=" * 70)
        print("🔄 V8.2.6 EXECUTABLE TEST-INTENT INTELLIGENCE")
        print("=" * 70)
    ...
    # ========================================================
    # V8.2.6 BEHAVIOR MODEL + BUSINESS JOURNEY DISCOVERY
    # ========================================================
    ...
    def _v826_build_behavior_model(self):
        ...
        self.v826_behavior_model={"version":"8.2.6","behaviors":behaviors,"journeys":journeys,
                                  "behavior_count":len(behaviors),"journey_count":len(journeys)}
        print("\n🧠 V8.2.6 BEHAVIOR MODEL")
    ...
    def _v826_write_behavior_artifact(self):
        try:
            path=REPORT_DIR/"qa_behavior_model_v8_2_6.json"
    ...
    async def run(self):
        ...
        # V8.2.6: construct behavioral entities and business-journey hypotheses.
        ...
        # V8.2.6: link execution evidence back to the behavior model.
        self._v826_sync_behavior_coverage()
        self._v826_write_behavior_artifact()

        # V8.2.11: actually execute high-risk unknown behaviors.
        ...
        self._v8211_finalize_truth()
        self._v8210_finalize_truth_gate()
        ...
        if mission_truth["status"] == "NOT_MEASURABLE":
            print("\n🛡️ V8.2.6 MISSION TRUTH GATE: PASS")
    ...
    #
    # V8.2.6 produces an auditable quality decision without
    # storing credentials, tokens, cookies, passwords, or secrets.
    # ========================================================

    def _v78_execution_truth(self):
        # V8.2.6: quality truth is derived from the same classified
    ...
    def _v78_write_quality_truth(self):
        ...
        try:
            ...
            print(
                f"\n⚠️ V8.2.6 quality-truth artifact "
                f"write skipped: {exc}"
            )
    ...
    def _v821_classified_execution_counts(self):
        """V8.2.6 authoritative intent-aware execution accounting.
        ...
        V8.2.6 uses the single V8 classifier so all reporting layers agree.
        """
        counts = {
        ...
        if counts["TOTAL"]["executed"] != len(executable):
            raise AssertionError(
                "V8.2.6 execution accounting mismatch: "
    ...
    def _v82_reconcile_execution_state(self):
        ...
        for item in results:
            ...
            # Use the same classifier as every other V8.2.6 truth layer.
            kind = self._v8_classify_test(item)
    ...
    def _v822_validate_execution_truth(self):
        ...
        if total["executed"] != total["passed"] + total["failed"]:
            raise AssertionError(
                "V8.2.6 invariant failed: executed != passed + failed"
            )
        ...
        for kind in ("SMOKE", "SANITY", "REGRESSION", "E2E"):
            ...
            if row["executed"] != row["passed"] + row["failed"]:
                raise AssertionError(
                    f"V8.2.6 invariant failed for {kind}: "
        ...
        if classified_generated != (
            ...
            raise AssertionError(
                "V8.2.6 generated-intent execution ledger mismatch"
            )
    ...
    def _v825_validate_behavioral_mission(self):
        ...
        if truth.get("status") == "MODEL_REQUIRED":
            print("\n🧠 V8.2.6 BEHAVIOR MODEL GATE: MODEL_REQUIRED")
...
async def main():
    args = _v79_parse_cli()
    url = args.url
    print("\n*** RUNNING qa_agent_v5_1_DEFECT_INVESTIGATION_FINAL.py — VERSION 8.2.4 FINAL ***\n")
