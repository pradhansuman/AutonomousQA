...
class V87Engine:
    ...
    async def discover(self, browser) -> bool:
        ...
        print("\n" + "=" * 70, flush=True)
        print("🗺️ V8.7 CURRENT-RUN DISCOVERY", flush=True)
        print("=" * 70, flush=True)
        print(f"Target     : {self.target}", flush=True)
        print(f"Run ID     : {self.run_id}", flush=True)
        print(f"Max pages  : {self.max_pages}", flush=True)
        print(f"Timeout ms : {self.timeout_ms}", flush=True)
        ...
        try:
            while queue and len(self._seen_urls) < self.max_pages:
                ...
                print(
                    f"🔎 V8.7 DISCOVER [{len(self._seen_urls)}] {url}",
                    flush=True,
                )
        ...
        print(
            f"📚 DISCOVERY COMPLETE | pages={len(self.run.pages)}",
            flush=True,
        )
    ...
    async def run_agent(self) -> int:
        ...
        print("\n" + "=" * 70, flush=True)
        print("🚀 V8.7 SINGLE CANONICAL TRUTH ENGINE", flush=True)
        print("=" * 70, flush=True)
        ...
        async with async_playwright() as p:
            try:
                ...
                self.run.discovery_status = "FAIL"
            ...
            try:
                ...
                if not discovered:
                    print(
                        "🔴 V8.7 STOP: current discovery did not establish "
                        "a canonical application model.",
                        flush=True,
                    )
                    truth = self.calculate_truth()
                    self.save_artifacts(truth)
