#!/usr/bin/env python3
"""
V8.7.12 - DISCOVERY ROBUSTNESS + SEMANTIC MODEL FIX
...
VERSION = "8.7.12"
REPORT_DIR = Path("qa_v8_7_12_report")
...
class Agent:
    ...
    async def navigate(self,page,url):
        ...
        navigation remains pending. V8.7.12 therefore separates:
          1. document commit,
          2. DOM attachment,
    ...
    async def discover(self,browser):
        self.log("="*70)
        self.log("🗺️ V8.7.12 CURRENT-RUN DISCOVERY")
    ...
    def build_behaviors(self):
        ...
        """
        self.log("="*70)
        self.log("🧠 V8.7.12 CANONICAL SEMANTIC BEHAVIOR MODEL")
    ...
    async def execute_safe_navigation(self,browser):
        ...
        V8.7.12 uses the Element attributes directly and deduplicates by
        canonical destination surface.
        """
        self.log("="*70)
        self.log("🧪 V8.7.12 SAFE NAVIGATION EXECUTION")
    ...
    async def execute_semantic_behaviors(self, browser):
        """Execute a bounded set of real semantic behaviors.

        V8.7.10 built 73 canonical behaviors but never executed them, so
        coverage stayed at 0%. V8.7.12 closes that gap.

        Rules:
          * Only current-run canonical behaviors are eligible.
          * Navigation is handled separately.
          * Risk is used for deterministic ordering.
          * Unknown-budget bounds execution.
          * Unsupported/unsafe actions remain UNKNOWN; they are never
            converted to PASS.
          * PASS means the UI action itself was successfully performed and
            observed, not that a backend/business outcome was proven.
        """
        self.log("="*70)
        self.log("🧪 V8.7.12 SEMANTIC BEHAVIOR EXECUTION")

        eligible = [
            b for b in self.behaviors
            if (
                b.get("current_run") is True
        ...
        for n, b in enumerate(selected, 1):
            ...
            try:
                ...
                # Discovery selectors can become stale/dynamic between
                # discovery and execution. Resolve the original selector
                # first, then use a semantic fallback based on the current DOM.
                loc = page.locator(selector).first
                try:
                    await loc.wait_for(
                        state="visible",
                        timeout=max(self.action_timeout, 5000)
                    )
                except Exception:
                    fallback = None

                    # Strong semantic fallbacks. Prefer accessible role/name
                    # over brittle generated IDs or generic tag selectors.
                    if semantic == "BUTTON":
                        if label and label.lower() not in ("button", "submit"):
                        ...
                        if fallback is None or await fallback.count() == 0:
                            fallback = page.locator(
                                "input:visible:not([type='hidden'])"
                            ).first

                    elif semantic == "TEXTAREA":
                        if label:
                            fallback = page.get_by_label(
                                label, exact=True
                            ).first
                        if fallback is None or await fallback.count() == 0:
                            fallback = page.locator("textarea:visible").first

                    elif semantic == "SELECT":
                        fallback = page.locator("select:visible").first

                    elif semantic == "COMBOBOX":
                        fallback = page.get_by_role("combobox").first

                    elif semantic == "CHECKBOX":
                        fallback = page.get_by_role("checkbox").first

                    elif semantic == "RADIO":
                        fallback = page.get_by_role("radio").first

                    elif semantic == "SLIDER":
                        fallback = page.get_by_role("slider").first
                        ...
                        if fallback is None or await fallback.count() == 0:
                            fallback = page.get_by_role("tab").first

                    if fallback is None:
                        raise RuntimeError(
                            f"element not visible and no semantic fallback: "
                            f"{selector!r}"
                        )
                    ...
                    loc = fallback
                    self.log(
                        f"   🔄 SELECTOR RECOVERED | "
                        f"{semantic} | {label[:60]}"
                    )

                before_url = canon(page.url, url)
                evidence_detail = {
                ...
                if semantic in ("TEXT_INPUT", "TEXTAREA"):
                    # Deterministic non-secret probe value. Do not submit.
                    value = "QA_V8_7_11_PROBE"
                    await loc.fill(value)
                    observed = await loc.input_value()
                    if observed != value:
                        raise RuntimeError(
                            f"input observation mismatch: {observed!r}"
                        )
                    evidence_detail["observed_value"] = value
                    ...
                    if not options:
                        raise RuntimeError("select has no options")
                    chosen = None
                    for option in options:
                        disabled = await option.is_disabled()
                        value = await option.get_attribute("value")
                        if not disabled and value is not None:
                            chosen = value
                            break
                    if chosen is None:
                        raise RuntimeError("no selectable option")
                    await loc.select_option(chosen)
                    selected_values = await loc.input_value()
                    if selected_values != chosen:
                        raise RuntimeError(
                            f"select observation mismatch: {selected_values!r}"
                        )
                    evidence_detail["selected_value"] = chosen

                elif semantic == "COMBOBOX":
                    await loc.fill("QA")
                    await page.wait_for_timeout(300)
                    # Observe the control after typing. We intentionally do not
                    # assert a business result or select an option, because a
                    # combobox's backend/business outcome is outside UI evidence.
                    observed = await loc.input_value()
                    if observed != "QA":
                        raise RuntimeError(
                            f"combobox observation mismatch: {observed!r}"
                        )
                    evidence_detail["observed_value"] = "QA"
                    ...
                    if before == after:
                        # A slider at max may legitimately not move. Verify
                        # that it is still an actual interactive control.
                        role = await loc.get_attribute("role")
                        typ = await loc.get_attribute("type")
                        if role != "slider" and typ != "range":
                            raise RuntimeError(
                                "slider produced no observable state change"
                            )
                    evidence_detail["before_value"] = before
                    evidence_detail["after_value"] = after

                elif semantic == "TAB":
                    await loc.click()
                    await page.wait_for_timeout(150)
                    evidence_detail["after_url"] = canon(page.url, url)

                elif semantic == "BUTTON":
                    # Click only the canonical control. Dialogs are allowed;
                    # no business/backend result is inferred.
                    dialog_messages = []
                    ...
                    evidence_detail["dialog_count"] = len(dialog_messages)
                    evidence_detail["after_url"] = canon(page.url, url)

                else:
                    skipped += 1
                    self.log(
                        f"   🟡 UNKNOWN REMAINS | unsupported semantic={semantic}"
                    )
                    continue
        ...
        self.log(
            f"🧪 SEMANTIC EXECUTION COMPLETE | "
            f"pass={passed} | fail={failed} | skipped={skipped} | "
            f"remaining_unknown={sum(1 for b in self.behaviors if b.get('status') == 'UNKNOWN')}"
        )
    ...
    def truth(self,discovery_ok):
        ...
        self.log("="*70)
        self.log("🧭 V8.7.12 SINGLE CANONICAL TRUTH")
        self.log("="*70)
        for k,v in truth.items():
            if k=="gates": continue
            self.log(f"{k.replace('_',' ').title():30}: {v}")
        self.log("-"*70)
        self.log("🛡️ V8.7.12 TRUTH GATES")
        self.log("-"*70)
        for k,v in gates.items():
            self.log(f"{k:30} : {'PASS' if v else 'FAIL'}")
        self.log("-"*70)
        self.log(
            "🟢 V8.7.12 RELEASE TRUTH GATE: PASS"
            if release else
            "🔴 V8.7.12 RELEASE TRUTH GATE: FAIL\n"
    ...
    def write(self,truth):
        ...
        (REPORT_DIR/"qa_v8_7_12_canonical_truth.json").write_text(
    ...
    async def run(self):
        self.log("="*70)
        self.log("🚀 V8.7.12 DISCOVERY ROBUSTNESS + SEMANTIC QA")
        self.log("="*70)
        ...
        async with async_playwright() as p:
            ...
            self.write(truth)
            await browser.close()
            self.log(f"📄 Canonical truth: {REPORT_DIR/'qa_v8_7_12_canonical_truth.json'}")
