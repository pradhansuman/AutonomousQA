...
# ============================================================================
# V8.5.3.7 — TRUE OBSERVABILITY / EVIDENCE CORRELATION
# ============================================================================
V852_VERSION = "8.5.2"
...
def _v85_write_truth(model, truth):
    d=Path("qa_v8_5_3_7_report"); d.mkdir(parents=True,exist_ok=True)
    mp=d/"qa_business_outcome_verification_v8_5_3_7.json"; tp=d/"qa_v8_5_3_7_business_outcome_truth.json"
    ...
    print("\n"+"="*70); print("🧭 V8.5.3.7 TRUE OBSERVABILITY / BUSINESS-OUTCOME TRUTH"); print("="*70)
    ...
    print(f"\n   {'🟢' if truth['pass'] else '🔴'} V8.5.3.7 EVIDENCE TRUTH GATE: {'PASS' if truth['pass'] else 'FAIL'}"); print("="*70); print(f"📄 Outcome verification: {mp}"); print(f"📄 Outcome truth: {tp}")


# ============================================================================
# V8.5.3.7 — FAIL-CLOSED AUTONOMOUS EXECUTION INTEGRITY
# ============================================================================
#
...
def _v853_write_integrity_artifact(payload):
    report_dir = Path("qa_v8_5_3_7_report")
    report_dir.mkdir(parents=True, exist_ok=True)
    path = report_dir / "qa_execution_integrity_v8_5_3_7.json"
    path.write_text(
...
def _v853_print_blocked_truth(payload, artifact):
    print("\n" + "=" * 70)
    print("🛡️ V8.5.3.7 FAIL-CLOSED EXECUTION INTEGRITY")
    print("=" * 70)
...
# ============================================================================
# V8.5.3.7 — TARGETED JOURNEY & UNKNOWN-BEHAVIOR CLOSURE
# ============================================================================
#
# Scope:
...
def _v8533_write_closure_artifact(summary):
    report_dir = Path("qa_v8_5_3_7_report")
    report_dir.mkdir(parents=True, exist_ok=True)
    path = report_dir / "qa_targeted_closure_v8_5_3_7.json"
    path.write_text(
...
async def _v8533_run_targeted_closure(agent):
    ...
    print("\n" + "=" * 70)
    print("🎯 V8.5.3.7 TARGETED JOURNEY & UNKNOWN-BEHAVIOR CLOSURE")
    print("=" * 70)
...
# ============================================================================
# V8.5.3.7 — FAILURE REPRODUCTION & ENVIRONMENT/APPLICATION CLASSIFICATION
# ============================================================================
#
...
def _v8534_write_reproduction_artifact(target, records, classification):
    ...
    path = report_dir / "qa_failure_reproduction_v8_5_3_7.json"
    path.write_text(
        json.dumps(payload, indent=2, ensure_ascii=False, default=str),
        encoding="utf-8",
    )
...
async def _v8534_failure_reproduction(agent, target):
    ...
    try:
        ...
        print("\n" + "=" * 70)
        print("🔬 V8.5.3.7 FAILURE REPRODUCTION")
        print("=" * 70)
...
# ============================================================================
# V8.5.3.7 — FAILURE RECORD NORMALIZATION / CRASH-PROOF REPORTING
# ============================================================================
...
# ============================================================================
# V8.5.3.7 — DETERMINISTIC FAILURE EVIDENCE & JOURNEY REPRODUCTION
# ============================================================================
...
def _v8536_write_evidence(url, records, classification):
    ...
    path = report_dir / "qa_failure_evidence_v8_5_3_7.json"
    path.write_text(
        json.dumps(payload, indent=2, ensure_ascii=False, default=str),
        encoding="utf-8",
    )
...
async def _v8536_entrypoint_reproduction(agent, target):
    ...
    if browser is None:
        ...
        print("🔴 V8.5.3.7: browser handle unavailable")
        return result
    ...
    try:
        ...
        print("\n" + "=" * 70)
        print("🔬 V8.5.3.7 DETERMINISTIC FAILURE EVIDENCE")
        print("=" * 70)
...
# ============================================================================
# V8.5.3.7 — CURRENT-RUN DISCOVERY / EXECUTION BARRIER
# ============================================================================
#
# HARD INVARIANT:
...
def _v8537_current_model_is_valid(discovery):
    if not isinstance(discovery, dict):
        return False, "DISCOVERY_RESULT_INVALID"
    ...
    if not isinstance(pages, (list, tuple)):
        return False, "CURRENT_MODEL_PAGES_MISSING"

    if len(pages) == 0:
        return False, "CURRENT_MODEL_EMPTY"

    target = str(discovery.get("target", "")).rstrip("/")
    if target:
        normalized = {
            str(
                p.get("url", p) if isinstance(p, dict) else p
            ).rstrip("/")
            for p in pages
        }
...
def _v8537_clear_legacy_execution_state(agent):
    """Prevent stale/legacy state from becoming executable evidence."""
    blocked = []
...
def _v8537_write_integrity_artifact(
    ...
    path = report_dir / "qa_execution_barrier_v8_5_3_7.json"
    path.write_text(
        json.dumps(payload, indent=2, ensure_ascii=False, default=str),
        encoding="utf-8",
    )
...
async def _v8537_establish_current_model(agent, target):
    """
    Establish a fresh model using the agent's own current discovery capability.
    This function intentionally does not read prior application-map files.
    """
    discovery = None

    # Try common discovery entry points already present in the agent.
    for name in (
        "_discover_application",
    ...
    if discovery is None:
        # Do not manufacture a model from stale artifacts.
        return {
            "ok": False,
            "reason": "CURRENT_DISCOVERY_API_UNAVAILABLE",
            "pages": [],
        }

    ok, reason = _v8537_current_model_is_valid(discovery)
    pages = (
        discovery.get("pages", [])
...
async def _v8537_execution_barrier(agent, target):
    """
    Hard gate before agent.run():
      discovery FAIL -> execution MUST NOT start
      discovery PASS -> execution may proceed
    """
    result = await _v8537_establish_current_model(agent, target)

    if not result["ok"]:
        blocked = _v8537_clear_legacy_execution_state(agent)
        artifact = _v8537_write_integrity_artifact(
            target=target,
            discovery_ok=False,
        ...
        print("\n" + "=" * 70)
        print("🛡️ V8.5.3.7 CURRENT-RUN DISCOVERY / EXECUTION BARRIER")
        print("=" * 70)
        print("   Discovery status : FAIL")
        print(f"   Reason           : {result['reason']}")
        print("   Current pages    : 0")
        print("   Tests executed   : 0")
        print("   Legacy state     : BLOCKED")
    ...
    # Make the fresh model the only model eligible for this run.
    try:
        agent.pages = result["pages"]
    except Exception:
        pass
    ...
    print(f"   Current pages    : {len(result['pages'])}")
...
async def main():
    ...
    print("\n*** RUNNING qa_agent_v8_5_3_7_FAIL_CLOSED_EXECUTION_INTEGRITY.py — VERSION 8.5.3.7 ***\n")

    if not url.startswith(("http://", "https://")):
        print("❌ Invalid URL")
        sys.exit(1)

    # CRITICAL V8.5.3.7 RULE:
    ...
    print("🛡️ V8.5.3.7 CURRENT-RUN DISCOVERY PREFLIGHT")
    ...
    if not _v853_connectivity_gate(preflight, target):
        ...
        print("🔴 V8.5.3.7 STOP: agent execution was NOT started.")
        raise SystemExit(3)
    ...
    barrier_ok, _barrier_result = await _v8537_execution_barrier(
        agent, target
    )
    if not barrier_ok:
        return
