#!/usr/bin/env python3
"""
V8.7.22 - DISCOVERY ROBUSTNESS + SEMANTIC MODEL FIX
...
class Agent:
    ...
    async def navigate(self,page,url):
        ...
        navigation remains pending. V8.7.22 therefore separates:
          1. document commit,
          2. DOM attachment,
    ...
    async def discover(self,browser):
        self.log("="*70)
        self.log("🗺️ V8.7.22 CURRENT-RUN DISCOVERY")
    ...
    def build_behaviors(self):
        ...
        """
        self.log("="*70)
        self.log("🧠 V8.7.22 CANONICAL SEMANTIC BEHAVIOR MODEL")
    ...
    async def execute_safe_navigation(self,browser):
        ...
        V8.7.22 uses the Element attributes directly and deduplicates by
        canonical destination surface.
        """
        self.log("="*70)
        self.log("🧪 V8.7.22 SAFE NAVIGATION EXECUTION")
    ...
    async def _resolve_locator(self, page, semantic, selector, label):
        """V8.7.22 current-DOM semantic resolver with bounded readiness/fallbacks."""
    ...
    async def _execute_one_semantic(self, page, loc, semantic, label):
        """V8.7.22 execute semantics and observe actual current UI state."""
        s = str(semantic or "").upper()
        ...
        if s == "FILE_UPLOAD":
            ...
            probe.write_text(
                "V8.7.22 current-run upload probe\n",
                encoding="utf-8"
            )
    ...
    async def execute_semantic_behaviors(self, browser):
        """V8.7.22 targeted semantic execution with bounded recovery and runtime semantic correction."""
        self.log("="*70)
        self.log("🧪 V8.7.22 TARGETED SEMANTIC EXECUTION")
        ...
        if unresolved:
            self.log(
                f"🔁 V8.7.22 CLOSURE PASS | unresolved={len(unresolved)}"
            )
        for b in unresolved:
            page = None
            semantic = str(b.get("semantic", "")).upper()
        ...
        self.log(
            f"🧪 V8.7.22 SEMANTIC EXECUTION COMPLETE | "
    ...
    def truth(self,discovery_ok):
        ...
        self.log("="*70)
        self.log("🧭 V8.7.22 SINGLE CANONICAL TRUTH")
        self.log("="*70)
        for k,v in truth.items():
            if k=="gates": continue
            self.log(f"{k.replace('_',' ').title():30}: {v}")
        self.log("-"*70)
        self.log("🛡️ V8.7.22 TRUTH GATES")
        self.log("-"*70)
        for k,v in gates.items():
            self.log(f"{k:30} : {'PASS' if v else 'FAIL'}")
        self.log("-"*70)
        self.log(
            "🟢 V8.7.22 RELEASE TRUTH GATE: PASS"
            if release else
            "🔴 V8.7.22 RELEASE TRUTH GATE: FAIL\n"
    ...
    async def run(self):
        self.log("="*70)
        self.log("🚀 V8.7.22 DISCOVERY ROBUSTNESS + SEMANTIC QA")
        self.log("="*70)
