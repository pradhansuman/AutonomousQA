#!/usr/bin/env python3
"""
V8.7.9 - DISCOVERY ROBUSTNESS + SEMANTIC MODEL FIX
...
VERSION = "8.7.9"
REPORT_DIR = Path("qa_v8_7_9_report")
...
class Agent:
    ...
    async def navigate(self,page,url):
        ...
        navigation remains pending. V8.7.9 therefore separates:
          1. document commit,
          2. DOM attachment,
    ...
    async def discover(self,browser):
        self.log("="*70)
        self.log("🗺️ V8.7.9 CURRENT-RUN DISCOVERY")
    ...
    def build_behaviors(self):
        self.log("="*70)
        self.log("🧠 V8.7.9 SEMANTIC BEHAVIOR MODEL")
        uniq={}
        for p in self.pages:
            for e in p.elements:
                risk={
    ...
    async def execute_safe_navigation(self,browser):
        ...
        V8.7.9 uses the Element attributes directly and deduplicates by
        canonical destination surface.
        """
        self.log("="*70)
        self.log("🧪 V8.7.9 SAFE NAVIGATION EXECUTION")
    ...
    def truth(self,discovery_ok):
        ...
        self.log("="*70)
        self.log("🧭 V8.7.9 SINGLE CANONICAL TRUTH")
        self.log("="*70)
        for k,v in truth.items():
            if k=="gates": continue
            self.log(f"{k.replace('_',' ').title():30}: {v}")
        self.log("-"*70)
        self.log("🛡️ V8.7.9 TRUTH GATES")
        self.log("-"*70)
        for k,v in gates.items():
            self.log(f"{k:30} : {'PASS' if v else 'FAIL'}")
        self.log("-"*70)
        self.log(
            "🟢 V8.7.9 RELEASE TRUTH GATE: PASS"
            if release else
            "🔴 V8.7.9 RELEASE TRUTH GATE: FAIL\n"
    ...
    def write(self,truth):
        ...
        (REPORT_DIR/"qa_v8_7_9_canonical_truth.json").write_text(
    ...
    async def run(self):
        self.log("="*70)
        self.log("🚀 V8.7.9 DISCOVERY ROBUSTNESS + SEMANTIC QA")
        self.log("="*70)
        ...
        async with async_playwright() as p:
            ...
            self.write(truth)
            await browser.close()
            self.log(f"📄 Canonical truth: {REPORT_DIR/'qa_v8_7_9_canonical_truth.json'}")
