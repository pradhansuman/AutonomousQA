#!/usr/bin/env python3
"""
V8.7.14 - DISCOVERY ROBUSTNESS + SEMANTIC MODEL FIX
...
VERSION = "8.7.14"
REPORT_DIR = Path("qa_v8_7_13_report")
...
class Agent:
    ...
    async def navigate(self,page,url):
        ...
        navigation remains pending. V8.7.14 therefore separates:
          1. document commit,
          2. DOM attachment,
    ...
    async def discover(self,browser):
        self.log("="*70)
        self.log("🗺️ V8.7.14 CURRENT-RUN DISCOVERY")
    ...
    def build_behaviors(self):
        ...
        """
        self.log("="*70)
        self.log("🧠 V8.7.14 CANONICAL SEMANTIC BEHAVIOR MODEL")
    ...
    async def execute_safe_navigation(self,browser):
        ...
        V8.7.14 uses the Element attributes directly and deduplicates by
        canonical destination surface.
        """
        self.log("="*70)
        self.log("🧪 V8.7.14 SAFE NAVIGATION EXECUTION")
    ...
    async def _resolve_locator(self, page, semantic, selector, label):
        ...
        if s == "BUTTON":
            ...
            candidates.append(("visible-input",
                page.locator(
                    "input:visible:not([type='hidden']):not([disabled])"
                ).first))
    ...
    async def _execute_one_semantic(self, page, loc, semantic, label):
        """V8.7.14 semantic executor.

        Rules:
        - Never claim backend/business success.
        - PASS only when current DOM state directly verifies the action.
        - FILE_UPLOAD uses an isolated temporary file.
        - SLIDER is treated as range input, never as text.
        - TEXT_INPUT validation is based on the actual DOM value.
        """
        import tempfile
        import os
        ...
        if s in ("TEXT_INPUT", "TEXTAREA"):
            ...
            # Range inputs are NOT text inputs.
            if input_type == "range":
                ...
                if before == after:
                    # Some browsers/pages need ArrowLeft as fallback.
                ...
                if before == after:
                    raise RuntimeError(
                        f"range input did not change | value={before!r}"
                    )

                return detail

            # Generate a value appropriate for the input.
            if input_type == "tel":
                value = "9876543210"
            elif input_type == "number":
                value = "42"
            elif input_type == "email":
                value = "qa_v8_7_14@example.test"
            else:
                value = "QA_V8_7_14_PROBE"
            ...
            if observed != value:
                # Do not immediately classify as an application defect.
                # Give controlled inputs a short opportunity to settle.
                await page.wait_for_timeout(150)
                observed = await loc.input_value()

            if observed != value:
                raise RuntimeError(
                    f"input observation mismatch: "
        ...
        if s == "FILE_UPLOAD":
            tag = (
                await loc.evaluate("(e)=>e.tagName")
            ).lower()
            ...
            if tag != "input" or input_type != "file":
                raise RuntimeError(
                    f"FILE_UPLOAD target is not file input | "
                    f"tag={tag!r} type={input_type!r}"
                )

            # Create an isolated harmless temporary file.
            ...
            try:
                os.write(
                    fd,
                    b"V8.7.14 CURRENT RUN UPLOAD PROBE\n"
                )
                os.close(fd)

                await loc.set_input_files(temp_path)

                # Direct browser-side evidence.
                ...
                if not files:
                    raise RuntimeError(
                        "file upload observation failed: "
                        "browser reports zero selected files"
                    )

                observed_name = files[0].get("name", "")

                if observed_name != os.path.basename(temp_path):
                    raise RuntimeError(
                        "file upload observation mismatch: "
                        f"expected={os.path.basename(temp_path)!r} "
                        f"observed={observed_name!r}"
                    )

                detail.update(
                    mode="file",
                    file_name=observed_name,
                    file_size=files[0].get("size"),
                    file_type=files[0].get("type"),
        ...
        if s == "CHECKBOX":
            ...
            if not after:
                raise RuntimeError(
                    "checkbox did not become checked"
                )
        ...
        if s == "SELECT":
            ...
            if tag == "select":
                ...
                if chosen is None:
                    raise RuntimeError(
                        "native select has no selectable option"
                    )
                ...
                if observed != chosen:
                    raise RuntimeError(
                        "native select observation mismatch: "
                        f"expected={chosen!r} "
            ...
            if observed != "A":
                raise RuntimeError(
                    "combobox observation mismatch: "
                    f"expected='A' observed={observed!r}"
                )
        ...
        if s == "COMBOBOX":
            ...
            if observed != "QA":
                raise RuntimeError(
                    "combobox observation mismatch: "
                    f"expected='QA' observed={observed!r}"
                )

            try:
                option_count = (
                    await page.get_by_role("option").count()
                )
            except Exception:
                option_count = 0
        ...
        if s == "SLIDER":
            ...
            if input_type != "range":
                raise RuntimeError(
                    f"SLIDER target is not range input | "
                    f"type={input_type!r}"
                )
            ...
            if before == after:
                # Retry once with keyboard interaction.
                await loc.press("ArrowLeft")
                await page.wait_for_timeout(100)
                after = await loc.input_value()
        ...
        if s == "TAB":
            ...
            detail.update(
                ...
                aria_selected=await loc.get_attribute(
                    "aria-selected"
                ),
            )
            return detail
        ...
        raise RuntimeError(
            f"unsupported semantic behavior: {s}"
        )

    async def execute_semantic_behaviors(self, browser):
        """V8.7.14 targeted semantic execution with bounded recovery."""
        self.log("="*70)
        self.log("🧪 V8.7.14 TARGETED SEMANTIC EXECUTION")
        ...
        self.log(
            f"🧪 V8.7.14 SEMANTIC EXECUTION COMPLETE | "
    ...
    def truth(self,discovery_ok):
        ...
        self.log("="*70)
        self.log("🧭 V8.7.14 SINGLE CANONICAL TRUTH")
        self.log("="*70)
        for k,v in truth.items():
            if k=="gates": continue
            self.log(f"{k.replace('_',' ').title():30}: {v}")
        self.log("-"*70)
        self.log("🛡️ V8.7.14 TRUTH GATES")
        self.log("-"*70)
        for k,v in gates.items():
            self.log(f"{k:30} : {'PASS' if v else 'FAIL'}")
        self.log("-"*70)
        self.log(
            "🟢 V8.7.14 RELEASE TRUTH GATE: PASS"
            if release else
            "🔴 V8.7.14 RELEASE TRUTH GATE: FAIL\n"
    ...
    async def run(self):
        self.log("="*70)
        self.log("🚀 V8.7.14 DISCOVERY ROBUSTNESS + SEMANTIC QA")
        self.log("="*70)
