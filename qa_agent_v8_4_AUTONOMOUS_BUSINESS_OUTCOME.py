...
def _v828_write_closure_artifact(self, closure):
    path = REPORT_DIR / "qa_unknown_behavior_closure_v8_3_1.json"
    payload = {
        "version": "8.3.2",
...
def _v829_write_provenance_artifact(report_dir, results):
    ...
    path = Path(report_dir) / "qa_execution_provenance_v8_3_1.json"
    path.write_text(
...
def _v8210_write_truth_artifact(report_dir, truth):
    path = Path(report_dir) / "qa_truth_gate_v8_3_1.json"
    path.write_text(
...
def _v8211_write_truth_artifact(report_dir, truth):
    path = Path(report_dir) / "qa_truth_single_source_v8_3_1.json"
    path.write_text(
        json.dumps(truth, indent=2, ensure_ascii=False, default=str),
        encoding="utf-8",
    )
...
""".format(
    ...
    def _v826_write_behavior_artifact(self):
        try:
            path=REPORT_DIR/"qa_behavior_model_v8_3_1.json"
...
def _v84_release_truth(outcome_model, journey_truth):
    ...
    journeys_ok = (
        journey_truth["journeys_failed"] == 0
        and journey_truth["journeys_executed"]
        == journey_truth["journeys_discovered"]
    )
...
async def main():
    args = _v79_parse_cli()
    url = args.url
    print("\n*** RUNNING qa_agent_v8_3_2_TRUE_CANONICAL.py — VERSION 8.3.2 FINAL ***\n")
