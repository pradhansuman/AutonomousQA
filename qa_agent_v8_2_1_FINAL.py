...
""".format(
    ...
    def _v70_print_mission(self):
        ...
        print("\n")
        print("=" * 70)
        print("🎯 V8.2.1 AUTONOMOUS QA MISSION")
        print("=" * 70)
    ...
    def _v76_write_artifact(self):
        try:
            (REPORT_DIR / "qa_change_intelligence_v8_2_1.json").write_text(
                json.dumps({
                    "version": "7.6",
                    "change_risk": getattr(self, "v76_change_risk", 0),
            ...
            print(f"\n📄 Change-intelligence artifact: {(REPORT_DIR / 'qa_change_intelligence_v8_2_1.json').absolute()}")
        except Exception as exc:
    ...
    #
    # V8.2.1 produces an auditable quality decision without
    ...
    def _v78_write_quality_truth(self):
        ...
        try:
            ...
            print(
                f"\n⚠️ V8.2.1 quality-truth artifact "
                f"write skipped: {exc}"
            )
    ...
    def _v821_classified_execution_counts(self):
        """Authoritative intent-aware execution accounting."""
        counts = {
        ...
        for item in getattr(self, "results", []) or []:
            ...
            kind = str(
                item.get(
                    "test_type",
                    item.get("type", "REGRESSION"),
                )
            ).upper()

            if kind not in counts:
                # Never silently call an unknown intent Regression.
                continue
        ...
        counts["TOTAL"] = {
            "executed": sum(v["executed"] for k, v in counts.items()),
            "passed": sum(v["passed"] for k, v in counts.items()),
            "failed": sum(v["failed"] for k, v in counts.items()),
        }
        return counts
...
async def main():
    args = _v79_parse_cli()
    url = args.url
    print("\n*** RUNNING qa_agent_v5_1_DEFECT_INVESTIGATION_FINAL.py — VERSION 8.2.1 FINAL ***\n")
